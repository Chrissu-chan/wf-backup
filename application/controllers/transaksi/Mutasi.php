...
class Mutasi extends BaseController
{
    ...
    public function index()
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            return $this->datatable->resource($this->mutasi_kasir_m)
                ->view('mutasi_kasir')
                ->where('mutasi_kasir.tipe', 'mutasi')
                ->where('status', $status)
        ...
        $this->load->view('transaksi/mutasi/index', $data);
    }

    public function view($id)
    {
        $model = $this->mutasi_kasir_m->view('mutasi_kasir')->find_or_fail($id);
        $this->load->view('transaksi/mutasi/view', array(
            'model' => $model
        ));
    }
    ...
    {
        $this->load->view('transaksi/mutasi/create');
    }
    ...
    public function store()
    {
        ...
        $id_shift_aktif_kasir = $this->shift_aktif_m->view('shift_aktif')->scope('cabang')->scope('aktif')->first()->id_shift_kasir_aktif;
        print_r($id_shift_aktif_kasir);
        exit;
        $result = $this->mutasi_kasir_m->insert($post);
        if ($result) {
            $response = array(
                'success' => true,
                'message' => $this->localization->lang('success_store_message', array('name' => $this->localization->lang('mutasi')))
            );
        } else {
            $response = array(
                'success' => false,
                'message' => $this->localization->lang('error_store_message', array('name' => $this->localization->lang('mutasi')))
            );
        }
    ...
    public function edit($id)
    {
        ...
        $this->load->view('transaksi/mutasi/edit', array(
            'model' => $model
        ));
    }
    ...
    public function update($id)
    {
        ...
        $result = $this->mutasi_kasir_m->update($id, $post);
        if ($result) {
            $response = array(
                'success' => true,
                'message' => $this->localization->lang('success_update_message', array('name' => $this->localization->lang('mutasi')))
            );
        } else {
            $response = array(
                'success' => false,
                'message' => $this->localization->lang('error_update_message', array('name' => $this->localization->lang('mutasi')))
            );
        }
        $this->output->set_content_type('application/json')->set_output(json_encode($response));
    }
    ...
    {
        $result = $this->mutasi_kasir_m->delete($id);
        if ($result) {
            $response = array(
                'success' => true,
                'message' => $this->localization->lang('success_delete_message', array('name' => $this->localization->lang('mutasi')))
            );
        } else {
            $response = array(
                'success' => false,
                'message' => $this->localization->lang('error_delete_message', array('name' => $this->localization->lang('mutasi')))
            );
        }
        $this->output->set_content_type('application/json')->set_output(json_encode($response));
    }
    ...
    public function appr()
    {
        ...
        foreach ($post as $id) {
            $result = $this->mutasi_kasir_m->appr($id);
        }
        if ($result) {
            $this->redirect->with('success_message', $this->localization->lang('success_approve_message', array('name' => $this->localization->lang('mutasi'))))->back();
        } else {
            $this->redirect->with('error_message', $this->localization->lang('error_approve_message', array('name' => $this->localization->lang('mutasi'))))->back();
        }
    }
    ...
    public function cancel_appr()
    {
        $post = $this->input->post('id');
        foreach ($post as $id) {
            $result = $this->mutasi_kasir_m->cancel_appr($id);
        }
        if ($result) {
            $this->redirect->with('success_message', $this->localization->lang('success_cancel_approve_message', array('name' => $this->localization->lang('mutasi'))))->back();
        } else {
            $this->redirect->with('error_message', $this->localization->lang('error_cancel_approve_message', array('name' => $this->localization->lang('mutasi'))))->back();
        }
    }

    public function upload($id)
    {
        $this->load->view('transaksi/mutasi/upload', array(
            'id' => $id
        ));
    }
