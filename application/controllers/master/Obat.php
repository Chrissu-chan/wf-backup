...
class Obat extends BaseController {
    ...
    public function view($id) {
        $model = $this->barang_obat_m->view('obat')->where('obat.id_barang', $id)->first_or_fail();
    ...
    public function update($id) {
        ...
        $this->transaction->start();
	        $post['minus'] = isset($post['minus']) ? $post['minus'] : 0;
	        $this->barang_obat_m->update($id, $post);
	        $this->obat_m->where('id_barang', $id)->update($post);
	        if (isset($post['satuan_barang'])) {
		        $id_satuan = $model->id_satuan;
		        $id_satuan_beli = 0;
		        $record_konversi_satuan = array();
		        $this->konversi_satuan_m->scope('konversi')
	            ...
	            foreach ($post['satuan_barang'] as $key => $satuan) {
	                if ($key == 0) {
		                ...
		                if (isset($satuan['id_satuan'])) {
			                ...
			                $record_konversi_satuan[] = array(
				                'id_satuan_asal' => $satuan['id_satuan'],
				                'id_satuan_tujuan' => $id_satuan,
				                'konversi' => $satuan['konversi']
			                );
	            ...
	            if ($record_konversi_satuan) {
		            $this->konversi_satuan_m->insert_batch($record_konversi_satuan);
	            }
