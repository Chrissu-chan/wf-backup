...
<?php $this->template->section('content') ?>
    ...
    <div class="panel panel-default">
        <div class="panel-body">
            <?= $this->form->open($this->route->name('inventory.stock_opname.finish', array('id' => $id_stock_opname)), 'id="frm-stock_opname"') ?>
            <div class="bwizard clearfix">
                ...
                <div class="form-group">
                    <button type="button" id="btn-finish" class="btn btn-success pull-right">{{finish}}</button>
                </div>
            </div>
            <?= $this->form->close(); ?>
        </div>
	...
	<script>
		$(function () {
			...
			$('#btn-finish').click(function () {
				swalConfirm('Apakah anda yakin akan mengakhiri stock opname?', function() {
					$('#frm-stock_opname').submit();
				});
			});
		});
	</script>
